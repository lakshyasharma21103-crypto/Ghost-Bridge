...
async function loadReservedInvocation({ connection, capabilityName, actor }) {
  ...
  const persistedCredentialBindingId = idOf(invocation.credentialBindingId);
  const connectionCredentialBindingId = idOf(connection.credentialBindingId);
  if (persistedCredentialBindingId !== connectionCredentialBindingId) {
    throw new AppError(
      409,
      ErrorCodes.CREDENTIAL_VALIDATION_FAILED,
      'Durable invocation credential binding no longer matches the Connection.',
      [],
      {
        invocationId: idOf(invocation),
        reasonCode: 'DURABLE_CREDENTIAL_BINDING_MISMATCH',
      },
    );
  }
  const persistedCredentialRequirement = invocation.credentialRequirement;
  const expectedCredentialRequirement = connectionCredentialBindingId
    ? {
        adapterId: connection.runtimeType,
        purpose: 'runtime_invocation',
      }
    : undefined;
  if (
    (expectedCredentialRequirement &&
      (persistedCredentialRequirement?.adapterId !== expectedCredentialRequirement.adapterId ||
        persistedCredentialRequirement?.purpose !== expectedCredentialRequirement.purpose)) ||
    (!expectedCredentialRequirement &&
      (persistedCredentialRequirement?.adapterId || persistedCredentialRequirement?.purpose))
  ) {
    throw new AppError(
      409,
      ErrorCodes.CREDENTIAL_VALIDATION_FAILED,
      'Durable invocation credential requirement no longer matches the runtime adapter.',
      [],
      {
        invocationId: idOf(invocation),
        reasonCode: 'DURABLE_CREDENTIAL_REQUIREMENT_MISMATCH',
      },
    );
  }
