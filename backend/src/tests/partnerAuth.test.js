...
test('partner auth accepts a matching stored API key hash', async () => {
  ...
  Partner.findOne = (filter) => ({
    select: () => ({
      lean: async () => {
        assert.equal(filter.apiKeyHash, hashPartnerApiKey(rawKey));
        return {
          _id: 'partner_123',
          slug: 'flowai',
          status: 'active',
...
test('a valid seeded partner key loads its safe identity and existing Research Test Agent passport', async () => {
  const rawKey = generatePartnerApiKey();
  const originalFindPartner = Partner.findOne;
  ...
  try {
    ...
    Partner.findOne = originalFindPartner;
