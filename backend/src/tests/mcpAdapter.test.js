...
test('Runtime Gateway rejects quarantined MCP invocations and persists a structured failed invocation', async () => {
  const patches = [];
  const audits = [];
...
test('connection views expose MCP as quarantined by default', () => {
  ...
  assert.equal(result.runtime.availability, 'unavailable');
