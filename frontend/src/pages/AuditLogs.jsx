import { EyeOff, RefreshCw, ScrollText, ShieldAlert } from 'lucide-react';
...
export function AuditLogs() {
  ...
  const loadLogs = useCallback(() => {
    ...
    apiClient
      .get(`/audit-logs?${new URLSearchParams(identity)}`)
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <SecondaryButton onClick={loadLogs} disabled={state.loading || !partnerConfigured}>
      ...
      {!partnerConfigured ? (
        ...
        >
          ...
          to load workspace audit records.
        </div>
      ) : null}
      ...
      <Panel>
        <div className="flex items-start gap-3 border-b border-slate-200 pb-5">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-600">
              Raw install keys, runtime grants, credentials, and token values are removed before
              ...
              return (
                <div key={event.id} className="flex gap-4 py-5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-sm leading-6 text-slate-600">
                      {event.entityType} / {metadataSummary(event.metadata)}
                    </p>
        ...
        {!state.loading && !state.items.length ? (
          <EmptyState
            ...
            detail="Resolve an install key or invoke a connected agent to create redacted gateway records."
          />
        ) : null}
      </Panel>
    </>
