...
export function assertClosedCoreObjectSchemas(entry, schema) {
  function scan(schemaNode, pointer, inheritedUnevaluatedClosure) {
    ...
    const sameInstanceClosure =
      inheritedUnevaluatedClosure || schemaNode.unevaluatedProperties === false;
    if (
    ...
    for (const keyword of ['allOf', 'anyOf', 'oneOf']) {
      visitSchemaArray(schemaNode[keyword], (child, index) =>
        scan(child, childPointer(pointer, keyword, index), sameInstanceClosure),
      );
    }
    for (const keyword of ['if', 'then', 'else', 'not']) {
      if (Object.hasOwn(schemaNode, keyword))
        scan(schemaNode[keyword], childPointer(pointer, keyword), sameInstanceClosure);
    }
    visitSchemaMap(schemaNode.dependentSchemas, (child, key) =>
      scan(child, childPointer(pointer, 'dependentSchemas', key), sameInstanceClosure),
    );

    for (const keyword of ['properties', 'patternProperties', '$defs', 'definitions']) {
      visitSchemaMap(schemaNode[keyword], (child, key) =>
        scan(child, childPointer(pointer, keyword, key), false),
      );
    }
    for (const keyword of ['prefixItems']) {
      visitSchemaArray(schemaNode[keyword], (child, index) =>
        scan(child, childPointer(pointer, keyword, index), false),
      );
    }
    for (const keyword of [
      ...
      if (Object.hasOwn(schemaNode, keyword))
        scan(schemaNode[keyword], childPointer(pointer, keyword), false);
    }
  }
...
export function validateAssetSchemas({ ajv, manifest, assets }) {
  ...
  if (!manifestValidator)
    fail(`Manifest schema is not preloaded: ${String(manifest.manifestSchema)}`);
  if (!manifestValidator(manifest))
    fail(`Manifest validation failed: ${ajv.errorsText(manifestValidator.errors)}`);
  ...
  for (const entry of [
    ...
    if (!validate(asset))
      fail(`Asset validation failed for ${entry.path}: ${ajv.errorsText(validate.errors)}`);
  }
}
...
export function assertValidatorImportIsolation(repositoryRoot, validationRoot) {
  ...
  for (const file of allFiles) {
    const absoluteFile = resolveRepositoryFilesystemPath(
      repositoryRoot,
      file,
      'file',
    ...
    for (const specifier of specifiers) {
      ...
      if (!specifier.startsWith('.'))
        fail(`Non-foundation validator import in ${file}: ${specifier}`);
